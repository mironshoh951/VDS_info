...
function paletteFor(rng: () => number, warm: boolean): Palette {
 ...
 */
export function monogramSvg(key: string, label: string, size = 512): string {
 ...
 */
export function sceneSvg(key: string, width = 2000, height = 1200): string {
  const rng = rngFor(key)
  ...
  for (let i = 0; i < count; i += 1) {
    const b = blob(
      `b${i}`,
      width * (0.08 + rng() * 0.88),
      height * (0.02 + rng() * 0.96),
      Math.min(width, height) * (0.32 + rng() * 0.5),
